...
const Button: React.FC<ButtonProps & React.ButtonHTMLAttributes<HTMLButtonElement>> = ({ 
  ...
  const variants = {
    primary: 'bg-button-1 text-global-4 hover:bg-[#e6003d] active:bg-[#cc0036] focus:ring-[#fc004e]',
  ...
  const responsiveRadius = 'rounded-[10px]';
  ...
  return (
    <button
      ...
      className={`
        ...
        max-h-[40px]
        touch-manipulation
        shadow-[2px_2px_10px_#00e7f9]
        pt-[8px] pr-[40px] pb-[8px] pl-[34px]
        ${className}
      `.trim().replace(/\s+/g, ' ')}
    ...
    >
      ...
      <span className={`${loading ? 'opacity-75' : ''} text-[20px] leading-[24px]`}>
        {children}
      </span>
      {rightIcon && (
        <Image
